...
Public Const HPISections = 0
Public Const HPIFeatures = 1
Public Const HPIAnims = 2
Public Const HPIObjects = 3
...
Public Sub CreateHPIList()
    ...
    For Index = 0 To UBound(StartupLocations)
        Path = ""
        Path = Dir(StartupLocations(Index))
        If StartupLocations(Index) <> "" And Path <> "" Then
            Path = StartupLocations(Index)
                ...
                If Name <> "." And Name <> ".." Then
                    If (GetAttr(Path & Name) And vbNormal) = vbNormal Then
                        If Right(Name, 3) = "hpi" Or Right(Name, 3) = "ufo" Or Right(Name, 3) = "gpf" Or Right(Name, 3) = "ccx" Or Right(Name, 3) = "gp3" Then
                            ...
                            HPINames(UBound(HPINames)) = StartupLocations(Index) & Name
                        End If
                    End If
                End If
    ...
    For Index = 1 To UBound(HPINames)
        FrmStartup.Update 0, Index, UBound(HPINames)
